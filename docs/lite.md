...
We will inspect filesystem images. We will reason about how filedata and metadata are stored and maintained. 

This is a light version of "p4: filesystem image forensics". 
...
Fiddling with a whole filesystem (and disk images) requires the root privilege. 

* Linux/Mac users: do this lab either on your local box which you have root. Or do it on the server, inside a QEMU emulator. The problem with QEMU: you will need a system image with all the utilities, e.g. dumpe2fs, etc. Can be built with buildroot which requires some exploration.
